...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This cmspagemanager no longer exists.'));
        ...
        $this->_coreRegistry->register('cmspagemanager', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Cmspagemanager') : __('New Item'),
            $id ? __('Edit Cmspagemanager') : __('New Item')
        );
