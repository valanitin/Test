 ...
 */
namespace Magentizer\GeoIP\Block\Store;
...
class Switcher extends \Magento\Framework\View\Element\Template
{
    ...
    public function getShippingCountryFromCookie(){
        $geoCountryId = $this->_cookieManager->getCookie("mb_shipping_country");
        if(isset($geoCountryId)){
            return $this->getCountryname($geoCountryId);
        }else{
            return "";
        }
    ...
    public function isShowPopup(){
        $geoshowpopup = $this->_cookieManager->getCookie("mb_show_popup");
        if(isset($geoshowpopup)){
            return $geoshowpopup;
        }else{
            return false;
        }
    ...
    public function getAllShippingCountries(){
        return $this->_addressHelper->getGroupCountryMappingForShipping();
    }
    
    public function getStoreByShipCountry($countryId){
        $DefaultSelection = $this->_addressHelper->getDefaultCountry($countryId);
              $cTstoreId = 0;
              if($DefaultSelection){
                $cTstoreId = $DefaultSelection["id"];
                }
                if($cTstoreId){
                    return $this->_storeManager->getStore($cTstoreId);
                }
                return false;
    }
}
