...
class TransactionCustomer implements \Swarming\StoreCredit\Api\TransactionCustomerInterface
{
    ...
    {
        $transactionData = (array)$this->request->getParam('transaction');
        
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        /* Create a new product object */
        $creditsCustomer = $objectManager->create('\Swarming\StoreCredit\Service\CreditsCustomer');

        $adjustmentdata = array(   'type' => $transactionData['type'],
                'amount' => $transactionData['amount'],
                'summary' => $transactionData['summary'] );

        try {
            $creditsCustomer->update($customerId,$adjustmentdata);
